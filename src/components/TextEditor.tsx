...
import { TextElement, fontOptions, colorPresets, CardCategory, categoryInfo } from '@/types/businessCard';
...
export const TextEditor = ({ texts, onUpdate, category, suggestedFonts }: TextEditorProps) => {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-4">
        ...
        <p className="text-sm text-muted-foreground">Click a text layer to edit, or add new text</p>
      </div>
        ...
        <div className="space-y-4 pt-4 border-t border-border">
          ...
          <div className="space-y-2">
            ...
            {suggestedFonts.length > 0 && (
              <p className="text-xs text-muted-foreground mb-2">✨ Suggested for {categoryInfo[category].name}</p>
            )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {colorPresets.slice(0, 16).map((color) => (
                <button
                  key={color}
                  onClick={() => updateStyle(selectedText.id, { color })}
                  className={cn(
